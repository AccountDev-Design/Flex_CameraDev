...
static esp_err_t photoHandler(httpd_req_t* req) {
  ...
  // Angulo de horizonte del instante del disparo. Sin esto el navegador
  // tendria que adivinarlo: entre que se pide la foto y llega el JPEG pasan
  // cientos de ms en los que la camara puede haberse movido.
  FcImuSample im; fcImuGet(&im);
  char hdrH[24], hdrV[4], hdrE[16];
  ...
  httpd_resp_set_hdr(req, "Access-Control-Expose-Headers",
                     "X-Horizon, X-Hvalid, X-Hepoch, X-FlexCam-Width, "
                     "X-FlexCam-Height, X-FlexCam-Bytes");
...
static void telemetryTask(void* arg) {
  ...
  for (;;) {
    ...
    if (s.seq != lastImuSequence || now - lastImuBroadcast >= 500) {
      lastImuSequence = s.seq;
      ...
      n = snprintf(buf, sizeof(buf),
          ...
          (unsigned long)s.resets, (unsigned long)s.epoch,
          (int)s.mountDeg, s.invert ? 1 : 0, (unsigned)s.plane);
...
static esp_err_t streamHandler(httpd_req_t* req) {
  ...
  while (true) {
    ...
    size_t hlen = snprintf(part, sizeof(part), STREAM_PART, (unsigned)frame.len,
                           (unsigned long)frame.capturedMs,
