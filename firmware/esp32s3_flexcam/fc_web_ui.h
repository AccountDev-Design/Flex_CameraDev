...
<div id="app">
  ...
  <section id="dock">
    ...
    <div id="panel">
      ...
      <div class="grid">
        <div class="kv"><div class="k">FPS captura</div><div class="v" id="vCapFps">—</div></div>
        <div class="kv"><div class="k">FPS enviados</div><div class="v" id="vSendFps">—</div></div>
        <div class="kv"><div class="k">FPS render</div><div class="v" id="vDrawFps">—</div></div>
        <div class="kv"><div class="k">Edad del frame</div><div class="v" id="vAge">—</div></div>
        <div class="kv"><div class="k">Latencia rel.</div><div class="v" id="vLat">—</div></div>
        <div class="kv"><div class="k">Frames perdidos</div><div class="v" id="vDrop">—</div></div>
...
// --------------------------- estado --------------------------------
const S = {
  modes:[], mode:null, srcW:800, srcH:600, camReady:false, sendFps:0,
  drop:0, heap:0, psram:0, temp:null,
...
function paintPanel(){
  ...
  $("vCapFps").textContent = S.capFps.toFixed(1);
  $("vSendFps").textContent= S.sendFps.toFixed(1);
  $("vDrawFps").textContent= S.drawFps.toFixed(0);
  ...
  $("vDrop").textContent   = S.drop;
  $("vTemp2").textContent  = S.temp===null? "—" : S.temp.toFixed(1)+" °C";
  $("vRs").textContent     = i.resets;
  $("vMem").textContent    = Math.round(S.heap/1024)+" KB internos · "+
                             Math.round(S.psram/1024)+" KB PSRAM";
}
function paintTop(){
...
function wsConnect(){
  ...
  ws.onmessage=ev=>{
    ...
    if(d.t==="i"){
      ...
      S.sendFps=d.fps; S.srcW=d.w; S.srcH=d.h; S.heap=d.heap; S.psram=d.ps;
      S.drop=d.drop; S.temp=d.tc; S.camReady=d.cam===1;
